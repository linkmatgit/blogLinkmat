...
utest:
	...
	$(dc) -f docker-compose.test.yml run phptest php bin/console --env=test doctrine:schema:update --force

.PHONY: test
test: vendor/autoload.php node_modules/time ## Execute les tests
	$(drtest) phptest vendor/bin/phpunit
.PHONY: dbtest
dbtest: vendor/autoload.php node_modules/time ## Execute les tests
	$(dc) -f docker-compose.test.yml run phptest php bin/console --env=test doctrine:database:drop --force
	$(dc) -f docker-compose.test.yml run phptest php bin/console --env=test doctrine:database:create
.PHONY: tt
